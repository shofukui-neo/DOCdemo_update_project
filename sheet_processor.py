...
async def auto_url_finder():
    ...
    try:
        ...
        print(f"⚠️ '{CREDENTIALS_JSON}' が見つかりません。")
        ans = input("Google Sheets連携を使わずに、テストモード（ダミーデータ）で続行しますか？ (y/n): ")
    ...
    async with async_playwright() as p:
        ...
        for i, row in enumerate(records, start=2): # records配列は0始まりだが、スプレッドシートの行はヘッダー（1行目）があるためデータは2行目から
            ...
            try:
                # 既存のlink_extractor.pyの関数を直接呼び出して連携し、結果を受け取る
                results = await extract_internal_links_and_screenshot(found_url)
                
                if results and not is_mock_mode and sh:
                    print("✅ スプレッドシートへ抽出結果を書き戻します...")
                    # 例: R列(18)＝スクショパス、S列(19)＝CSVパス、T列(20)＝リンク数、C列(3)＝ステータス
                    sh.update_cell(i, 18, results.get("screenshot_path", ""))
                    sh.update_cell(i, 19, results.get("csv_path", ""))
                    sh.update_cell(i, 20, results.get("total_links", 0))
                    sh.update_cell(i, 3, "完了")
                    print("✅ スプレッドシート更新完了！")
                elif is_mock_mode:
                    print("（テストモードのためスプレッドシートへの更新はスキップします）")
