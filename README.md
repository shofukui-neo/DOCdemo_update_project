...
- 生成完了待機は **最低15秒 + スピナー消失 + 「FAQを生成中…」/「企業情報を生成中…」テキスト消失** の AND 条件 (2026-05-19 改善)
  - 旧実装はスピナー消失だけで完了判定し、Streamlit の info ボックス表示 (生成中テキスト) を見逃して早期にFAQ実体検証に進み、結果として `FAQ実体がページに現れません` エラーが頻発していた
- FAQ 実体検証は **「生成中…」表示が消えてから純粋に180秒** を待つ (生成中の時間はカウントしない)
  - 生成本体が固まったケースは `CONTENT_GENERATION_TIMEOUT` を別カウンタで監視
- `save_content` 冒頭にも生成中テキスト残存ガードを追加し、生成完了前のクリック事故を防止
